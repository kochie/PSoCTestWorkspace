...
/* `#END` */
    ...
    *******************************************************************************/
    CY_ISR(UART_USB_RXISR)
    {
    ...
    #ifdef UART_USB_RXISR_ENTRY_CALLBACK
        UART_USB_RXISR_EntryCallback();
    #endif /* UART_USB_RXISR_ENTRY_CALLBACK */
        ...
        do
        {
            ...
            readStatus = UART_USB_RXSTATUS_REG;
            /* Copy the same status to readData variable for backward compatibility support 
            *  of the user code in UART_USB_RXISR_ERROR` section. 
            */
            readData = readStatus;

            if((readStatus & (UART_USB_RX_STS_BREAK | 
                            UART_USB_RX_STS_PAR_ERROR |
                            UART_USB_RX_STS_STOP_ERROR | 
                            UART_USB_RX_STS_OVERRUN)) != 0u)
            {
                /* ERROR handling. */
                UART_USB_errorStatus |= readStatus & ( UART_USB_RX_STS_BREAK | 
                                                            UART_USB_RX_STS_PAR_ERROR | 
                                                            UART_USB_RX_STS_STOP_ERROR | 
                                                            UART_USB_RX_STS_OVERRUN);
                /* `#START UART_USB_RXISR_ERROR` */

                /* `#END` */
                
            #ifdef UART_USB_RXISR_ERROR_CALLBACK
                UART_USB_RXISR_ERROR_Callback();
            #endif /* UART_USB_RXISR_ERROR_CALLBACK */
            }
            
            if((readStatus & UART_USB_RX_STS_FIFO_NOTEMPTY) != 0u)
            {
                ...
                {
                    if((readStatus & UART_USB_RX_STS_MRKSPC) != 0u)
                    {
                        if ((readStatus & UART_USB_RX_STS_ADDR_MATCH) != 0u)
                        {
                ...
                if(increment_pointer != 0u)
                {
                    ...
                    if(UART_USB_rxBufferWrite >= UART_USB_RX_BUFFER_SIZE)
                    {
                    ...
                    if(UART_USB_rxBufferWrite == UART_USB_rxBufferRead)
                    {
                        ...
                        #endif /* (UART_USB_FLOW_CONTROL != 0u) */
                    }
                }
            }
        }while((readStatus & UART_USB_RX_STS_FIFO_NOTEMPTY) != 0u);
    ...
    #ifdef UART_USB_RXISR_EXIT_CALLBACK
        UART_USB_RXISR_ExitCallback();
    #endif /* UART_USB_RXISR_EXIT_CALLBACK */
    ...
    *******************************************************************************/
    CY_ISR(UART_USB_TXISR)
    {
    ...
    #ifdef UART_USB_TXISR_ENTRY_CALLBACK
        UART_USB_TXISR_EntryCallback();
    #endif /* UART_USB_TXISR_ENTRY_CALLBACK */
             ...
             ((UART_USB_TXSTATUS_REG & UART_USB_TX_STS_FIFO_FULL) == 0u))
        {
            ...
            if(UART_USB_txBufferRead >= UART_USB_TX_BUFFER_SIZE)
            {
    ...
    #ifdef UART_USB_TXISR_EXIT_CALLBACK
        UART_USB_TXISR_ExitCallback();
    #endif /* UART_USB_TXISR_EXIT_CALLBACK */
