...
#if defined(CYDEV_BOOTLOADER_IO_COMP) && (0u != ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_USB) || \
         ...
         
{
    ...
    while(bufIndex < size)
    {
        UART_USB_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        */
        if(0u != UART_USB_GetRxBufferSize())
        {
            ...
            */
            do
            {
                oldDataCount = UART_USB_GetRxBufferSize();
                CyDelay(UART_USB_BYTE2BYTE_TIME_OUT);
            }
            while(UART_USB_GetRxBufferSize() > oldDataCount);

            status = CYRET_SUCCESS;
            break;
        }
        /* If the data is not received, give a delay of 
        *  UART_USB_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(UART_USB_BL_CHK_DELAY_MS);
        }
    }
    ...
    *count = 0u;
    ...
    while(UART_USB_GetRxBufferSize() > 0u)
    {
        tempCount = UART_USB_GetRxBufferSize();
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            for (iCntr = 0u; iCntr < tempCount; iCntr++)
            {
                ...
                pData[dataIndexCntr] = UART_USB_ReadRxData();
                dataIndexCntr++;
            }

            /* Check if the last received byte is end of packet defined by bootloader
            *  If not wait for additional UART_USB_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != UART_USB_PACKET_EOP)
            {
                CyDelay(UART_USB_WAIT_EOP_DELAY);
            }
        }
