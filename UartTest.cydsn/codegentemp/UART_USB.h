...
#define UART_USB_CLK_FREQ                       (0u)
#define UART_USB_TX_BUFFER_SIZE                 (4u)
#define UART_USB_RX_BUFFER_SIZE                 (4u)
...
#if defined(UART_USB_BUART_sCR_SyncCtl_CtrlReg__CONTROL_REG)
...
typedef struct UART_USB_backupStruct_
{
    uint8 enableState;
...
/* Only if RX is enabled */
        ...
        #define UART_USB_EnableRxInt()  CyIntEnable (UART_USB_RX_VECT_NUM)
        #define UART_USB_DisableRxInt() CyIntDisable(UART_USB_RX_VECT_NUM)
...
/* Only if TX is enabled */
        ...
        #define UART_USB_EnableTxInt()  CyIntEnable (UART_USB_TX_VECT_NUM)
        #define UART_USB_DisableTxInt() CyIntDisable(UART_USB_TX_VECT_NUM)
        #define UART_USB_SetPendingTxInt() CyIntSetPending(UART_USB_TX_VECT_NUM)
        #define UART_USB_ClearPendingTxInt() CyIntClearPending(UART_USB_TX_VECT_NUM)
...
/* Communication bootloader APIs */
#if defined(CYDEV_BOOTLOADER_IO_COMP) && ((CYDEV_BOOTLOADER_IO_COMP == CyBtldr_UART_USB) || \
                                          (CYDEV_BOOTLOADER_IO_COMP == CyBtldr_Custom_Interface))
    /* Physical layer functions */
    void    UART_USB_CyBtldrCommStart(void) CYSMALL ;
    void    UART_USB_CyBtldrCommStop(void) CYSMALL ;
    void    UART_USB_CyBtldrCommReset(void) CYSMALL ;
    cystatus UART_USB_CyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
             ;
    cystatus UART_USB_CyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut) CYSMALL
             ;
    ...
    #define UART_USB_BYTE2BYTE_TIME_OUT (25u)
    #define UART_USB_PACKET_EOP         (0x17u) /* End of packet defined by bootloader */
    #define UART_USB_WAIT_EOP_DELAY     (5u)    /* Additional 5ms to wait for End of packet */
    #define UART_USB_BL_CHK_DELAY_MS    (1u)    /* Time Out quantity equal 1mS */
...
/* Status Register definitions */
#if( (UART_USB_TX_ENABLED) || (UART_USB_HD_ENABLED) )
    #if(UART_USB_TX_INTERRUPT_ENABLED)
        #define UART_USB_TX_VECT_NUM            (uint8)UART_USB_TXInternalInterrupt__INTC_NUMBER
        #define UART_USB_TX_PRIOR_NUM           (uint8)UART_USB_TXInternalInterrupt__INTC_PRIOR_NUM
    #endif /* UART_USB_TX_INTERRUPT_ENABLED */
    ...
    #else /* (UART_USB_HD_ENABLED) */
        #define UART_USB_TX_STS_FIFO_FULL_SHIFT     (0x05u)  /* Needs MD=0 */
    #endif /* (UART_USB_TX_ENABLED) */

    #define UART_USB_TX_STS_COMPLETE            (uint8)(0x01u << UART_USB_TX_STS_COMPLETE_SHIFT)
    #define UART_USB_TX_STS_FIFO_EMPTY          (uint8)(0x01u << UART_USB_TX_STS_FIFO_EMPTY_SHIFT)
    #define UART_USB_TX_STS_FIFO_FULL           (uint8)(0x01u << UART_USB_TX_STS_FIFO_FULL_SHIFT)
    #define UART_USB_TX_STS_FIFO_NOT_FULL       (uint8)(0x01u << UART_USB_TX_STS_FIFO_NOT_FULL_SHIFT)
#endif /* End (UART_USB_TX_ENABLED) || (UART_USB_HD_ENABLED)*/

#if( (UART_USB_RX_ENABLED) || (UART_USB_HD_ENABLED) )
    #if(UART_USB_RX_INTERRUPT_ENABLED)
        #define UART_USB_RX_VECT_NUM            (uint8)UART_USB_RXInternalInterrupt__INTC_NUMBER
        #define UART_USB_RX_PRIOR_NUM           (uint8)UART_USB_RXInternalInterrupt__INTC_PRIOR_NUM
    #endif /* UART_USB_RX_INTERRUPT_ENABLED */
    ...
    #define UART_USB_RX_STS_MRKSPC           (uint8)(0x01u << UART_USB_RX_STS_MRKSPC_SHIFT)
    #define UART_USB_RX_STS_BREAK            (uint8)(0x01u << UART_USB_RX_STS_BREAK_SHIFT)
    #define UART_USB_RX_STS_PAR_ERROR        (uint8)(0x01u << UART_USB_RX_STS_PAR_ERROR_SHIFT)
    #define UART_USB_RX_STS_STOP_ERROR       (uint8)(0x01u << UART_USB_RX_STS_STOP_ERROR_SHIFT)
    #define UART_USB_RX_STS_OVERRUN          (uint8)(0x01u << UART_USB_RX_STS_OVERRUN_SHIFT)
    #define UART_USB_RX_STS_FIFO_NOTEMPTY    (uint8)(0x01u << UART_USB_RX_STS_FIFO_NOTEMPTY_SHIFT)
    #define UART_USB_RX_STS_ADDR_MATCH       (uint8)(0x01u << UART_USB_RX_STS_ADDR_MATCH_SHIFT)
    #define UART_USB_RX_STS_SOFT_BUFF_OVER   (uint8)(0x01u << UART_USB_RX_STS_SOFT_BUFF_OVER_SHIFT)
...
#define UART_USB_CTRL_HD_SEND               (uint8)(0x01u << UART_USB_CTRL_HD_SEND_SHIFT)
#define UART_USB_CTRL_HD_SEND_BREAK         (uint8)(0x01u << UART_USB_CTRL_HD_SEND_BREAK_SHIFT)
#define UART_USB_CTRL_MARK                  (uint8)(0x01u << UART_USB_CTRL_MARK_SHIFT)
#define UART_USB_CTRL_PARITY_TYPE_MASK      (uint8)(0x03u << UART_USB_CTRL_PARITY_TYPE0_SHIFT)
#define UART_USB_CTRL_RXADDR_MODE_MASK      (uint8)(0x07u << UART_USB_CTRL_RXADDR_MODE0_SHIFT)
...
#define UART_USB_BIT_CENTER                         (UART_USB_OVER_SAMPLE_COUNT - 2u)

#define UART_USB_FIFO_LENGTH                        (4u)
#define UART_USB_NUMBER_OF_START_BIT                (1u)
#define UART_USB_MAX_BYTE_VALUE                     (0xFFu)
...
#define UART_USB_HALF_BIT_COUNT   \
                            (((UART_USB_OVER_SAMPLE_COUNT / 2u) + (UART_USB_USE23POLLING * 1u)) - 2u)
    ...
    #define UART_USB_RXBITCTR_INIT  ((((UART_USB_BREAK_BITS_RX + UART_USB_NUMBER_OF_START_BIT) \
                            * UART_USB_OVER_SAMPLE_COUNT) + UART_USB_HALF_BIT_COUNT) - 1u)
    ...
    #define UART_USB_RXBITCTR_INIT      (((7u * UART_USB_OVER_SAMPLE_COUNT) - 1u) + \
                                                      UART_USB_HALF_BIT_COUNT)
#endif /* End UART_USB_OVER_SAMPLE_COUNT */

#define UART_USB_HD_RXBITCTR_INIT                   UART_USB_RXBITCTR_INIT
...
extern uint8 UART_USB_initVar;
    ...
    extern volatile uint8 UART_USB_txBuffer[UART_USB_TX_BUFFER_SIZE];
    ...
    extern uint8 UART_USB_errorStatus;
    extern volatile uint8 UART_USB_rxBuffer[UART_USB_RX_BUFFER_SIZE];
...
#define UART_USB_NUMBER_OF_STOP_BITS    (1u)
    ...
    #define UART_USB_RX_HW_ADDRESS1     (0u)
    #define UART_USB_RX_HW_ADDRESS2     (0u)
#endif /* (UART_USB_RXHW_ADDRESS_ENABLED) */

#define UART_USB_INIT_RX_INTERRUPTS_MASK \
                                  (uint8)((1 << UART_USB_RX_STS_FIFO_NOTEMPTY_SHIFT) \
...
#define UART_USB_INIT_TX_INTERRUPTS_MASK \
                                  (uint8)((0 << UART_USB_TX_STS_COMPLETE_SHIFT) \
...
#define UART_USB_WAIT_1_MS      UART_USB_BL_CHK_DELAY_MS   

#define UART_USB_TXBUFFERSIZE   UART_USB_TX_BUFFER_SIZE
#define UART_USB_RXBUFFERSIZE   UART_USB_RX_BUFFER_SIZE

#if (UART_USB_RXHW_ADDRESS_ENABLED)
    #define UART_USB_RXADDRESSMODE  UART_USB_RX_ADDRESS_MODE
    #define UART_USB_RXHWADDRESS1   UART_USB_RX_HW_ADDRESS1
    #define UART_USB_RXHWADDRESS2   UART_USB_RX_HW_ADDRESS2
