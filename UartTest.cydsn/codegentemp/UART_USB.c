...
uint8 UART_USB_initVar = 0u;
...
#if (UART_USB_TX_INTERRUPT_ENABLED && UART_USB_TX_ENABLED)
    volatile uint8 UART_USB_txBuffer[UART_USB_TX_BUFFER_SIZE];
    volatile uint8 UART_USB_txBufferRead = 0u;
    uint8 UART_USB_txBufferWrite = 0u;
#endif /* (UART_USB_TX_INTERRUPT_ENABLED && UART_USB_TX_ENABLED) */

#if (UART_USB_RX_INTERRUPT_ENABLED && (UART_USB_RX_ENABLED || UART_USB_HD_ENABLED))
    uint8 UART_USB_errorStatus = 0u;
    volatile uint8 UART_USB_rxBuffer[UART_USB_RX_BUFFER_SIZE];
    ...
    volatile uint8 UART_USB_rxBufferOverflow   = 0u;
        ...
        volatile uint8 UART_USB_rxAddressMode = UART_USB_RX_ADDRESS_MODE;
        volatile uint8 UART_USB_rxAddressDetected = 0u;
    #endif /* (UART_USB_RXHW_ADDRESS_ENABLED) */
#endif /* (UART_USB_RX_INTERRUPT_ENABLED && (UART_USB_RX_ENABLED || UART_USB_HD_ENABLED)) */
...
*  UART_USB_Start() sets the initVar variable, calls the
*  UART_USB_Init() function, and then calls the
...
*  and set to one (1u) the first time UART_USB_Start() is called. This
*  allows for component initialization without re-initialization in all
*  subsequent calls to the UART_USB_Start() routine.
*
* Reentrant:
*  No.
*
...
*  dialog settings. It is not necessary to call UART_USB_Init() because
*  the UART_USB_Start() API calls this function and is the preferred
...
void UART_USB_Init(void) 
{
            ...
            (void) CyIntSetVector(UART_USB_RX_VECT_NUM, &UART_USB_RXISR);
            CyIntSetPriority(UART_USB_RX_VECT_NUM, UART_USB_RX_PRIOR_NUM);
            UART_USB_errorStatus = 0u;
        #endif /* (UART_USB_RX_INTERRUPT_ENABLED) */

        #if (UART_USB_RXHW_ADDRESS_ENABLED)
            UART_USB_SetRxAddressMode(UART_USB_RX_ADDRESS_MODE);
            UART_USB_SetRxAddress1(UART_USB_RX_HW_ADDRESS1);
            UART_USB_SetRxAddress2(UART_USB_RX_HW_ADDRESS2);
        #endif /* End UART_USB_RXHW_ADDRESS_ENABLED */

        /* Init Count7 period */
        UART_USB_RXBITCTR_PERIOD_REG = UART_USB_RXBITCTR_INIT;
        ...
        UART_USB_RXSTATUS_MASK_REG  = UART_USB_INIT_RX_INTERRUPTS_MASK;
            ...
            (void) CyIntSetVector(UART_USB_TX_VECT_NUM, &UART_USB_TXISR);
        ...
        #else
            UART_USB_TXBITCTR_PERIOD_REG = ((UART_USB_NUMBER_OF_DATA_BITS +
                        UART_USB_NUMBER_OF_START_BIT) * UART_USB_OVER_SAMPLE_8) - 1u;
        ...
        UART_USB_WriteControlRegister( \
            (UART_USB_ReadControlRegister() & (uint8)~UART_USB_CTRL_PARITY_TYPE_MASK) | \
            (uint8)(UART_USB_PARITY_TYPE << UART_USB_CTRL_PARITY_TYPE0_SHIFT) );
...
*  to call UART_USB_Enable() because the UART_USB_Start() API
...
void UART_USB_Enable(void) 
{
    ...
    enableInterrupts = CyEnterCriticalSection();
        ...
        #if (UART_USB_TX_INTERRUPT_ENABLED)
            UART_USB_ClearPendingTxInt(); /* Clear history of TX_NOT_EMPTY */
            UART_USB_EnableTxInt();
        #endif /* (UART_USB_TX_INTERRUPT_ENABLED) */
     #endif /* (UART_USB_TX_INTERRUPT_ENABLED) */

    #if (UART_USB_INTERNAL_CLOCK_USED)
        UART_USB_IntClock_Start();  /* Enable the clock */
    #endif /* (UART_USB_INTERNAL_CLOCK_USED) */
...
void UART_USB_Stop(void) 
{
    ...
    #if (UART_USB_RX_ENABLED || UART_USB_HD_ENABLED)
        UART_USB_RXBITCTR_CONTROL_REG &= (uint8) ~UART_USB_CNTR_ENABLE;
    #endif /* (UART_USB_RX_ENABLED || UART_USB_HD_ENABLED) */

    #if (UART_USB_TX_ENABLED)
        #if(!UART_USB_TXCLKGEN_DP)
            UART_USB_TXBITCTR_CONTROL_REG &= (uint8) ~UART_USB_CNTR_ENABLE;
        #endif /* (!UART_USB_TXCLKGEN_DP) */
    #endif /* (UART_USB_TX_ENABLED) */

    #if (UART_USB_INTERNAL_CLOCK_USED)
        UART_USB_IntClock_Stop();   /* Disable the clock */
    #endif /* (UART_USB_INTERNAL_CLOCK_USED) */

    /* Disable internal interrupt component */
    #if (UART_USB_RX_ENABLED || UART_USB_HD_ENABLED)
        UART_USB_RXSTATUS_ACTL_REG  &= (uint8) ~UART_USB_INT_ENABLE;
    ...
    #if (UART_USB_TX_ENABLED)
        UART_USB_TXSTATUS_ACTL_REG &= (uint8) ~UART_USB_INT_ENABLE;
...
uint8 UART_USB_ReadControlRegister(void) 
{
    ...
    #endif /* (UART_USB_CONTROL_REG_REMOVED) */
}
...
void  UART_USB_WriteControlRegister(uint8 control) 
{
    ...
    *      UART_USB_RX_STS_FIFO_NOTEMPTY    Interrupt on byte received.
    *      UART_USB_RX_STS_PAR_ERROR        Interrupt on parity error.
    *      UART_USB_RX_STS_STOP_ERROR       Interrupt on stop error.
    *      UART_USB_RX_STS_BREAK            Interrupt on break.
    *      UART_USB_RX_STS_OVERRUN          Interrupt on overrun error.
    *      UART_USB_RX_STS_ADDR_MATCH       Interrupt on address match.
    *      UART_USB_RX_STS_MRKSPC           Interrupt on address detect.
    *
    * Return:
    *  None.
    *
    ...
    *  UART_USB_rxBufferLoopDetect - cleared if loop condition was detected
    *     in RX ISR.
    *
    * Reentrant:
    *  No.
    ...
    *******************************************************************************/
    uint8 UART_USB_ReadRxData(void) 
    {
        ...
        locRxBufferRead  = UART_USB_rxBufferRead;
        locRxBufferWrite = UART_USB_rxBufferWrite;

        if( (UART_USB_rxBufferLoopDetect != 0u) || (locRxBufferRead != locRxBufferWrite) )
        {
            rxData = UART_USB_rxBuffer[locRxBufferRead];
            locRxBufferRead++;

            if(locRxBufferRead >= UART_USB_RX_BUFFER_SIZE)
            {
                locRxBufferRead = 0u;
            }
            /* Update the real pointer */
            UART_USB_rxBufferRead = locRxBufferRead;
            ...
            if(UART_USB_rxBufferLoopDetect != 0u)
            {
                UART_USB_rxBufferLoopDetect = 0u;
                    ...
                    /* When Hardware Flow Control selected - return RX mask */
                        ...
                        if((UART_USB_CONTROL_REG & UART_USB_CTRL_HD_SEND) == 0u)
                ...
                #endif /* ((UART_USB_RX_INTERRUPT_ENABLED) && (UART_USB_FLOW_CONTROL != 0u)) */
            }
        }
        else
    ...
    *  UART_USB_RX_STS_FIFO_NOTEMPTY.
    *  UART_USB_RX_STS_FIFO_NOTEMPTY clears immediately after RX data
    *  register read.
    *
    * Global Variables:
    ...
    *******************************************************************************/
    uint8 UART_USB_ReadRxStatus(void) 
    {
        ...
        status = UART_USB_RXSTATUS_REG & UART_USB_RX_HW_MASK;

    #if (UART_USB_RX_INTERRUPT_ENABLED)
        if(UART_USB_rxBufferOverflow != 0u)
        {
    ...
    *
    * Summary:
    *  Returns the last received byte of data. UART_USB_GetChar() is
    ...
    *******************************************************************************/
    uint8 UART_USB_GetChar(void) 
    {
        ...
        {   rxStatus = UART_USB_RXSTATUS_REG;
            if((rxStatus & UART_USB_RX_STS_FIFO_NOTEMPTY) != 0u)
    ...
    *******************************************************************************/
    uint16 UART_USB_GetByte(void) 
    {
        ...
        /* Protect variables that could change on interrupt */
        UART_USB_DisableRxInt();
        locErrorStatus = (uint16)UART_USB_errorStatus;
        UART_USB_errorStatus = 0u;
        UART_USB_EnableRxInt();
        return ( (uint16)(locErrorStatus << 8u) | UART_USB_ReadRxData() );
    #else
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (UART_USB_RX_BUFFER_SIZE - UART_USB_rxBufferRead) + UART_USB_rxBufferWrite;
        }
        ...
        size = ((UART_USB_RXSTATUS_REG & UART_USB_RX_STS_FIFO_NOTEMPTY) != 0u) ? 1u : 0u;
    ...
    *******************************************************************************/
    void UART_USB_ClearRxBuffer(void) 
    {
        ...
        /* Clear the HW FIFO */
        enableInterrupts = CyEnterCriticalSection();
        UART_USB_RXDATA_AUX_CTL_REG |= (uint8)  UART_USB_RX_FIFO_CLR;
        UART_USB_RXDATA_AUX_CTL_REG &= (uint8) ~UART_USB_RX_FIFO_CLR;
        CyExitCriticalSection(enableInterrupts);
    ...
    *******************************************************************************/
                                                        ...
                                                        
    {
                ...
                uint8 tmpCtrl;
                tmpCtrl = UART_USB_CONTROL_REG & (uint8)~UART_USB_CTRL_RXADDR_MODE_MASK;
                tmpCtrl |= (uint8)(addressMode << UART_USB_CTRL_RXADDR_MODE0_SHIFT);
    ...
    *   UART_USB_TX_STS_COMPLETE        Interrupt on TX byte complete
    *   UART_USB_TX_STS_FIFO_EMPTY      Interrupt when TX FIFO is empty
    *   UART_USB_TX_STS_FIFO_FULL       Interrupt when TX FIFO is full
    *   UART_USB_TX_STS_FIFO_NOT_FULL   Interrupt when TX FIFO is not full
    *
    * Return:
    *  None.
    *
    ...
    *******************************************************************************/
    void UART_USB_WriteTxData(uint8 txDataByte) 
    {
        ...
        if(UART_USB_initVar != 0u)
        {
            ...
            {
                if(UART_USB_txBufferWrite >= UART_USB_TX_BUFFER_SIZE)
                {
    ...
    *******************************************************************************/
    void UART_USB_PutChar(uint8 txDataByte) 
    {
        ...
        */
        ...
        #if ((UART_USB_TX_BUFFER_SIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3))
            /* Disable TX interrupt to protect variables from modification */
            UART_USB_DisableTxInt();
        #endif /* (UART_USB_TX_BUFFER_SIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3) */

            locTxBufferWrite = UART_USB_txBufferWrite;
            locTxBufferRead  = UART_USB_txBufferRead;
        ...
        while( (locTxBufferWrite < locTxBufferRead) ? (locTxBufferWrite == (locTxBufferRead - 1u)) :
                                ...
                                (uint8)(UART_USB_TX_BUFFER_SIZE - 1u)) );
        ...
        {
            if(locTxBufferWrite >= UART_USB_TX_BUFFER_SIZE)
            {
                locTxBufferWrite = 0u;
            }
            /* Add to the software buffer. */
            UART_USB_txBuffer[locTxBufferWrite] = txDataByte;
            locTxBufferWrite++;
            ...
            UART_USB_txBufferWrite = locTxBufferWrite;

        #if ((UART_USB_TX_BUFFER_SIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3))
            UART_USB_EnableTxInt();
        #endif /* (UART_USB_TX_BUFFER_SIZE > UART_USB_MAX_BYTE_VALUE) && (CY_PSOC3) */

            if(0u != (UART_USB_TXSTATUS_REG & UART_USB_TX_STS_FIFO_EMPTY))
            {
                /* Trigger TX interrupt to send software buffer */
                UART_USB_SetPendingTxInt();
            }
        }

    #else

        while((UART_USB_TXSTATUS_REG & UART_USB_TX_STS_FIFO_FULL) != 0u)
        {
    ...
    *******************************************************************************/
    void UART_USB_PutString(const char8 string[]) 
    {
        ...
        if(UART_USB_initVar != 0u)
        {
            ...
            {
                UART_USB_PutChar((uint8)string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        if(UART_USB_initVar != 0u)
        {
            ...
            {
                UART_USB_PutChar(string[bufIndex]);
                bufIndex++;
            }
        }
    }
    ...
    *******************************************************************************/
                                                            ...
                                                            
    {
        ...
        {
            size = (UART_USB_TX_BUFFER_SIZE - UART_USB_txBufferRead) +
                    UART_USB_txBufferWrite;
        }
        ...
        /* Is the fifo is full. */
        if((size & UART_USB_TX_STS_FIFO_FULL) != 0u)
        {
            size = UART_USB_FIFO_LENGTH;
        }
        else if((size & UART_USB_TX_STS_FIFO_EMPTY) != 0u)
        {
            size = 0u;
        }
        else
    ...
    *******************************************************************************/
    void UART_USB_ClearTxBuffer(void) 
    {
        ...
        enableInterrupts = CyEnterCriticalSection();
        /* Clear the HW FIFO */
        UART_USB_TXDATA_AUX_CTL_REG |= (uint8)  UART_USB_TX_FIFO_CLR;
        UART_USB_TXDATA_AUX_CTL_REG &= (uint8) ~UART_USB_TX_FIFO_CLR;
        CyExitCriticalSection(enableInterrupts);
    ...
    *   UART_USB_SEND_BREAK - Initialize registers for break, send the Break
    *       signal and return immediately.
    *   UART_USB_WAIT_FOR_COMPLETE_REINIT - Wait until break transmission is
    *       complete, reinitialize registers to normal transmission mode then return
    *   UART_USB_REINIT - Reinitialize registers to normal transmission mode
    *       then return.
    *   UART_USB_SEND_WAIT_REINIT - Performs both options: 
    *      UART_USB_SEND_BREAK and UART_USB_WAIT_FOR_COMPLETE_REINIT.
    ...
    *     SendBreak(0);     - initialize Break signal transmission
    *         Add your code here to use CPU time
    *     When interrupt appear with UART_USB_TX_STS_COMPLETE status:
    *     SendBreak(2);     - complete Break operation
    *
    * Side Effects:
    *  The UART_USB_SendBreak() function initializes registers to send a
    *  break signal.
    ...
    void UART_USB_SendBreak(uint8 retMode) 
    {
        ...
        if(UART_USB_initVar != 0u)
        {
                ...
                (retMode == UART_USB_SEND_WAIT_REINIT ) )
            {
                ...
                while((tmpStat & UART_USB_TX_STS_FIFO_EMPTY) != 0u);
            }
                ...
                (retMode == UART_USB_SEND_WAIT_REINIT) )
            {
                ...
                while(((uint8)~tmpStat & UART_USB_TX_STS_COMPLETE) != 0u);
            }
            ...
            {
                UART_USB_WriteControlRegister(UART_USB_ReadControlRegister() &
                                              (uint8)~UART_USB_CTRL_HD_SEND_BREAK);
            }
                ...
                (retMode == UART_USB_SEND_WAIT_REINIT) )
            {
                /* CTRL_HD_SEND_BREAK - skip to send parity bit at Break signal in Full Duplex mode */
                                    ...
                                    (UART_USB_PARITY_TYPE_SW != 0u) )
                    UART_USB_WriteControlRegister(UART_USB_ReadControlRegister() |
                                                          UART_USB_CTRL_HD_SEND_BREAK);
                #endif /* End UART_USB_PARITY_TYPE != UART_USB__B_UART__NONE_REVB  */

                #if(UART_USB_TXCLKGEN_DP)
                    txPeriod = UART_USB_TXBITCLKTX_COMPLETE_REG;
                    UART_USB_TXBITCLKTX_COMPLETE_REG = UART_USB_TXBITCTR_BREAKBITS;
                #else
                    txPeriod = UART_USB_TXBITCTR_PERIOD_REG;
                ...
                (retMode == UART_USB_SEND_WAIT_REINIT) )
            {
            ...
            #if(UART_USB_TXCLKGEN_DP)
                UART_USB_TXBITCLKTX_COMPLETE_REG = txPeriod;
            #else
                UART_USB_TXBITCTR_PERIOD_REG = txPeriod;
            #endif /* End UART_USB_TXCLKGEN_DP */
                ...
                UART_USB_WriteControlRegister(UART_USB_ReadControlRegister() &
                                                      (uint8) ~UART_USB_CTRL_HD_SEND_BREAK);
            #endif /* End UART_USB_PARITY_TYPE != NONE */
            }
        #endif    /* End UART_USB_HD_ENABLED */
        }
    }
    ...
    * Parameters:
    *  addressMode: 
    *       UART_USB_SET_SPACE - Configure the transmitter to send the next
    *                                    byte as a data.
    *       UART_USB_SET_MARK  - Configure the transmitter to send the next
    *                                    byte as an address.
    *
    * Return:
    *  None.
    *
    * Side Effects:
    *  This function sets and clears UART_USB_CTRL_MARK bit in the Control
    *  register.
    *
    *******************************************************************************/
    void UART_USB_SetTxAddressMode(uint8 addressMode) 
    {
        ...
        if(addressMode != 0u)
        {
            ...
            UART_USB_WriteControlRegister(UART_USB_ReadControlRegister() |
                                                  UART_USB_CTRL_MARK);
        #endif /* End UART_USB_CONTROL_REG_REMOVED == 0u */
        }
        else
        {
        #if( UART_USB_CONTROL_REG_REMOVED == 0u )
            UART_USB_WriteControlRegister(UART_USB_ReadControlRegister() &
                                                  (uint8) ~UART_USB_CTRL_MARK);
        #endif /* End UART_USB_CONTROL_REG_REMOVED == 0u */
        }
    }
    ...
    *******************************************************************************/
    void UART_USB_LoadTxConfig(void) 
    {
        ...
        UART_USB_SetRxInterruptMode(0u);
    #endif /* (UART_USB_RX_INTERRUPT_ENABLED) */
